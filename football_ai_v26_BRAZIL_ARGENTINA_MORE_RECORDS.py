...
EXACT_ENGINE_VERSION = "32.0-VILA-NOVA-GOIAS-SPORT-RECIFE-PONTE-PRETA"
...
HISTORY_SEASONS = int(os.getenv("HISTORY_SEASONS", "10"))
...
def _load_targeted_exact_history(existing):
    """Merge targeted recent history for the two manually watched teams."""
    targets = []
    try:
    ...
    # 1) Team-specific recent result (free v1 gives only a tiny slice).
    for team in dict.fromkeys(targets):
        x = _tsdb_team_history(team, limit=50)
        if not x.empty:
            frames.append(x)

    # 2) Season-level TheSportsDB fallback.  This can recover several more
    # matches even when the free team endpoint returns only one home match.
    league_ids = {
        "ligue 1": 4334,
        "ligue1": 4334,
        "laliga": 4335,
    ...
    league_seasons = {
        4334: [f"{CURRENT_SEASON}-{CURRENT_SEASON + 1}", f"{CURRENT_SEASON - 1}-{CURRENT_SEASON}"],
        4335: [f"{CURRENT_SEASON}-{CURRENT_SEASON + 1}", f"{CURRENT_SEASON - 1}-{CURRENT_SEASON}"],
    }
    for lid, seasons in league_seasons.items():
        for season_label in seasons:
            x = _tsdb_league_season_history(lid, season_label)
            if x.empty:
                continue
    ...
    # 3) API-Football fallback. It is used only when the free TheSportsDB
    # sample is still too small for a team.
    per_team_counts = {}
    if frames:
        tmp = pd.concat(frames, ignore_index=True)
        for key in target_keys:
            per_team_counts[key] = int(((tmp["home_key"] == key) | (tmp["away_key"] == key)).sum())
    for team in dict.fromkeys(targets):
        if per_team_counts.get(norm_team(team), 0) >= 5:
            continue
        x = _api_team_history(team, limit=20)
        if not x.empty:
            frames.append(x)
