...
EXACT_ENGINE_VERSION = "35.0-BRAZIL-20HOME-20AWAY-5H2H-TOP5-TOP7"
...
def _api_team_history(team_name, limit=10):
    """Fallback team history from API-Football using accessible seasons.

    The current season may be unavailable on some API-Football plans. We try
    the current and previous season first, then 2024 as the known free-plan
    compatibility fallback. This data is marked separately so diagnostics can
    distinguish fresh from older evidence.
    """
    ...
    try:
        ...
        # Prefer current/recent season, but do not burn requests once a usable
        # season has returned enough matches.
        seasons = []
        for y in (CURRENT_SEASON, CURRENT_SEASON - 1, 2024):
            if y not in seasons:
                seasons.append(y)
        ...
        for season in seasons:
            ...
            if rows:
                # A successful recent season is enough; avoid mixing stale
                # 2024 data into a better current-season sample.
                if season >= CURRENT_SEASON - 1 and len(rows) >= min(5, int(limit)):
                    break
                if season == 2024 and len(rows) >= min(5, int(limit)):
                    break
...
def _load_targeted_exact_history(existing):
    ...
    # API-Football remains a fallback if a particular team is still below the
    # requested venue sample.  It is never used to fabricate missing records.
    if frames:
    ...
    for team in dict.fromkeys(targets):
        ...
        if home_count >= 20 and away_count >= 20:
            continue
        x = _api_team_history(team, limit=30)
        if not x.empty:
            frames.append(x)
...
def predict(df, home, away, league=None, match_date=None):
    ...
    return {
        ...
        "data_quality": {
            ...
            "targeted_live_history": int((df.get("source", pd.Series(dtype=str)).astype(str) == "thesportsdb-history").sum()) if hasattr(df, "get") else 0,
        },
    }
