...
def _pair_rank(pair):
    """Rank a 2-leg AKO by its weakest leg first, then overall quality."""
    ...
    return (min(confs), sum(confs) / 2.0, min(probs), sum(agreements) / 2.0)
...
def _pair_strictly_better(new_pair, old_pair):
    """A replacement must be at least as strong on every aggregate level and improve materially."""
    new_rank = _pair_rank(new_pair)
    old_rank = _pair_rank(old_pair)
    not_weaker = all(n >= o for n, o in zip(new_rank, old_rank))
    meaningful = (
        new_rank[0] >= old_rank[0] + EXACT_PAIR_REPLACEMENT_EDGE
        or new_rank[1] >= old_rank[1] + EXACT_PAIR_REPLACEMENT_EDGE
        or new_rank[2] >= old_rank[2] + EXACT_PAIR_REPLACEMENT_EDGE
    )
...
def process_hourly_signal(chat_id=None, force_first=False):
    """Hourly quality-control loop with transparent rejection diagnostics."""
    history = load_history()
    if history.empty or len(history) < 200:
        send(
            "⏰ KONTROLA GODZINNA\n\n"
            "❌ Brak wystarczającej historii. Słabe sygnały pozostają odrzucone.",
            chat_id,
        )
        return False
    ...
    if state is None or _pair_is_expired(state):
        ...
        if pair and len(pair) == 2:
            ...
            send(
                "⏰ KONTROLA GODZINNA\n\n"
                f"📊 Przeanalizowano: {len(candidates) + len(rejected)} meczów\n"
                f"❌ Odrzucono: {len(rejected)} słabych sygnałów\n"
        ...
        send(
            ...
            f"🔎 Przeanalizowano: {len(candidates) + len(rejected)} meczów\n"
            f"❌ Odrzucono: {len(rejected)} słabych sygnałów\n"
            "🔒 Nie znaleziono 2 wyników spełniających wszystkie rygorystyczne warunki.\n"
            "Słabe typy NIE zostały wysłane.",
            chat_id,
        )
        return False
    ...
    if improved:
        _save_pair_state(fresh, reason="stronger_global_pair")
        save_last(fresh, coupon_score(fresh))
        save_predictions(fresh, coupon_score(fresh))
        send(
            _pair_summary_message(fresh, "🚨 ZNALEZIONO RZECZYWIŚCIE MOCNIEJSZY SYGNAŁ!"),
            chat_id,
        )
        send(
            "⏰ KONTROLA GODZINNA\n\n"
            f"📊 Przeanalizowano: {len(candidates) + len(rejected)} meczów\n"
            f"❌ Odrzucono: {len(rejected)} słabych sygnałów\n"
            "🟢 Nowy AKO jest co najmniej tak mocny na każdej mierzonej osi "
            "i wyraźnie lepszy jako całość.\n"
            f"{signal_level(_pair_strength(fresh))}",
            chat_id,
        )
        return True

    # Keep current pair even if its fixtures temporarily disappear from the
    # strong-candidate pool; never replace a good signal with a weaker one.
    send(
        "⏰ KONTROLA GODZINNA\n\n"
        f"🔎 Przeanalizowano: {len(candidates) + len(rejected)} meczów\n"
        f"❌ Odrzucono: {len(rejected)} słabych sygnałów\n"
        "🔒 NIE ZNALEZIONO LEPSZEGO TYPU.\n"
        "✅ Żaden nowy zestaw nie spełnił warunku: równie mocny na wszystkich "
        "kluczowych kryteriach i jednocześnie wyraźnie lepszy.\n"
        f"{signal_level(_pair_strength(current))}\n\n"
        "🎟️ Obecny AKO pozostaje bez zmian: 2 dokładne wyniki na tych samych 2 meczach.",
        chat_id,
    )
    return False
...
def status_message():
    ...
    return (
        ...
        f"⏰ Kontrola: co 60 minut — pełny skan nadchodzących meczów\n"
        f"🔒 Podmiana AKO: tylko na równie mocny lub wyraźnie mocniejszy zestaw\n"
