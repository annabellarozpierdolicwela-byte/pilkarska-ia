...
EXACT_ENGINE_VERSION = "50.0-BRAZIL-SERIE-B-JUVENTUDE-ATHLETIC-NOVORIZONTINO-CUIABA-32H-32A-H2H"
...
def _load_targeted_exact_history(existing):
    """Merge deep FlashScore/Apify history for the four watched teams (Juventude, Athletic Club MG, Novorizontino, Cuiaba) and H2H.

    API-Football is deliberately not used here because the user's account is
    suspended. The Apify actor supplies up to 100 recent matches per team and
    available H2H in with-history mode. Odds are ignored completely.
    """
    targets = []
    try:
        ...
        targets = WATCHED_TEAMS
    targets = list(dict.fromkeys(targets))
    target_keys = {norm_team(t) for t in targets}

    frames = []
    # FIRST: deterministic local database. This is the stable core and does
    # not depend on API-Football, FlashScore availability, or a fresh Apify run.
    static_df = _static_team_database_df()
    if not static_df.empty:
        static_relevant = static_df[
            static_df["home_key"].isin(target_keys) | static_df["away_key"].isin(target_keys)
        ].copy()
        if not static_relevant.empty:
            frames.append(static_relevant)

    # Primary source: FlashScore historical snapshots through Apify.
    apify_df = _apify_fetch_target_history()
    if not apify_df.empty:
        apify_df["home_key"] = apify_df["home"].map(norm_team)
        apify_df["away_key"] = apify_df["away"].map(norm_team)
        relevant = apify_df[
            apify_df["home_key"].isin(target_keys) | apify_df["away_key"].isin(target_keys)
        ].copy()
        if not relevant.empty:
            frames.append(relevant)

    # Primary targeted Brazil Serie B supplement:
    # collect REAL venue-specific history for every watched team.
    # Requirement: 32 HOME + 32 AWAY per team when the source contains enough data.
    if os.getenv("EXACT_DEEP_TSDB_REFRESH", "0") == "1":
        ...
        log.warning("EXACT DEEP TSDB: pominięto odświeżanie 32H/32A w gorącej ścieżce /typy; ustaw EXACT_DEEP_TSDB_REFRESH=1 dla ręcznego odświeżenia.")

    # Small free-source supplement; never fabricate missing matches.
    for team in targets:
        x = _tsdb_team_history(team, limit=5)
        if not x.empty:
            frames.append(x)
    ...
    log.warning(
        "STATIC TEAM DB: %d verified rows loaded for Juventude/Athletic Club MG/Novorizontino/Cuiaba",
        len(static_df) if not static_df.empty else 0,
    )
    ...
    combined = pd.concat([existing, extra], ignore_index=True) if existing is not None and not existing.empty else extra
    combined["date"] = pd.to_datetime(combined["date"], utc=True, errors="coerce")
    combined["home_key"] = combined["home"].map(norm_team)
    combined["away_key"] = combined["away"].map(norm_team)
    combined = combined.dropna(subset=["date", "hg", "ag"])
    combined = combined.drop_duplicates(subset=["date", "home_key", "away_key"], keep="last")
    combined = combined.sort_values("date").reset_index(drop=True)

    for team in targets:
        k = norm_team(team)
        hh = int((combined["home_key"] == k).sum())
        aa = int((combined["away_key"] == k).sum())
        log.warning("FLASHSCORE MIN32 PREFLIGHT %s => HOME=%d AWAY=%d", team, hh, aa)
    # Pair-specific H2H diagnostics.
    for a, b in ((targets[0], targets[1]), (targets[2], targets[3])):
        ak, bk = norm_team(a), norm_team(b)
        h2h_n = int(((combined["home_key"] == ak) & (combined["away_key"] == bk) | (combined["home_key"] == bk) & (combined["away_key"] == ak)).sum())
        log.warning("FLASHSCORE H2H PREFLIGHT %s-%s => %d", a, b, h2h_n)

    try:
        combined.to_csv(HISTORY_CACHE, index=False)
    except Exception:
        pass
    return combined
...
def _exact_watch_predictions(history):
    ...
    for _, r in fixtures.iterrows():
        try:
            ...
            pred["min20_blocked"] = False
            pred["min20_missing"] = missing
            pred["min20_counts"] = required_counts
            if missing:
                log.warning(
                    "MIN20 DATA SHORT %s-%s: %s — analysis continues",
...
def process_exact_watch(chat_id=None, update=False):
    ...
    if len(all_predictions) < 2:
        ...
        for p in all_predictions:
            if p.get("min20_blocked"):
                c = p.get("min20_counts", {})
                diag_lines.append(
    ...
    if not combinations:
        debug = []
        for p in selected:
            debug.append(f"{p.get('home','?')}–{p.get('away','?')}: SCORE={len(p.get('scores') or [])}, INTERNAL={len(p.get('internal_scores') or [])}, MIN20={p.get('min20_blocked', False)}")
        send(
