...
EXACT_WATCH_INTERNAL_SCORES = int(os.getenv("EXACT_WATCH_INTERNAL_SCORES", "8"))
...
EXACT_ENGINE_VERSION = "11.0-ADAPTIVE"
...
def _manual_exact_watch_fixtures():
    """Four user-selected fixtures; the model chooses the two strongest exact-score candidates."""
    today = now().date()
    rows = [
        {"home": "Viking FK", "away": "Dinamo Zagreb", "league": "UEFA Champions League Qualifying", "status": "NS", "date": f"{today}T19:00:00+00:00", "source": "manual-exact-watch", "context": {"type": "two_leg_tie", "first_leg_home_goals": 2, "first_leg_away_goals": 2}},
        {"home": "AEK Athens", "away": "Levski Sofia", "league": "UEFA Champions League Qualifying", "status": "NS", "date": f"{today}T19:00:00+00:00", "source": "manual-exact-watch", "context": {"type": "two_leg_tie", "first_leg_home_goals": 0, "first_leg_away_goals": 0}},
        {"home": "Lyon", "away": "Fenerbahce", "league": "UEFA Champions League Qualifying", "status": "NS", "date": f"{today}T19:00:00+00:00", "source": "manual-exact-watch", "context": {"type": "two_leg_tie", "first_leg_home_goals": 1, "first_leg_away_goals": 1}},
        {"home": "Real Madrid", "away": "Real Sociedad", "league": "LaLiga", "status": "NS", "date": f"{today}T19:00:00+00:00", "source": "manual-exact-watch", "context": {"type": "league_match"}},
    ]
    df = pd.DataFrame(rows)
    df["date"] = pd.to_datetime(df["date"], utc=True, errors="coerce")
    df["home_key"] = df["home"].map(norm_team)
...
def exact_score_intelligence_rank(pred, context=None):
    """
    Ensemble exact-score engine.

    It deliberately does NOT use bookmaker odds. A score survives only when
    several independent views agree: blended Poisson/empirical model,
    Dixon-Coles correction, contextual adjustment and conservative historical
    self-calibration. Geometric blending penalizes scores that are strong in
    only one component.
    """
    rows = []
    ...
    if not base:
        for item in pred.get("top_scores", []):
            ...
            rows.append({
                **item,
                "intelligence": float(item.get("prob", 0.0)),
                "rank_probability": float(item.get("prob", 0.0)),
                "stability": 0.0,
                "context_multiplier": 1.0,
            })
        return sorted(rows, key=lambda x: x["intelligence"], reverse=True)
    ...
    for item in base:
        ...
        ctx = exact_score_context_multiplier(
            home, away, h, a_score, context
        )
        calibration = _exact_score_calibration_factor(home, away, score)

        # Geometric ensemble: disagreement between models is punished.
        geo = math.exp(
            0.42 * math.log(blend) +
            0.23 * math.log(poisson_p) +
            0.20 * math.log(empirical_p) +
            0.15 * math.log(dc_p)
        )

        # A high-quality exact score should also have a stable surrounding
        # distribution rather than relying on one isolated calculation.
        stability = (
            0.34 * min(geo / 0.12, 1.0) +
            0.24 * agreement +
            0.14 * min(margin / 0.04, 1.0) +
            0.14 * history_quality +
            0.14 * pattern_quality
        )
...
def _exact_watch_predictions(history):
    """Analyze all four fixtures, then rank fixtures by exact-score strength."""
    ...
    for _, r in fixtures.iterrows():
        try:
            ...
            ranked = exact_score_intelligence_rank(pred, context=r.get("context") or {})
            ...
            # Fixture score rewards the strongest exact score, model stability and separation
            # from the next candidate. It never uses bookmaker odds.
            ...
            # The fixture must have a concentrated, internally consistent exact-score
            # distribution. This is the key "selection" layer, not a bookmaker-price layer.
            fixture_quality = (
                top_prob *
                (0.62 + 0.18 * stability +
                 0.10 * min(1.0, margin / 0.08) +
                 0.10 * min(1.0, top3_mass / 0.45))
            )
            result.append({
...
def _exact_watch_message(all_predictions, selected, combinations, update=False, errors=0, new_combinations=None, new_signals=None):
    ...
    lines = [title, f"📅 {now().strftime('%d.%m.%Y %H:%M')} ({TZ})", cal_line, "", "🔎 RANKING 4 MECZÓW — EXACT DOUBLE INTELLIGENCE (BEZ KURSÓW)"]
    ...
    for i, pred in enumerate(selected, 1):
        ...
        for j, sc in enumerate(pred["scores"], 1):
            lines.append(f"   {j}. {sc['score']} — 🧠 ranking AI {float(sc.get('rank_probability',0.0)):.2%} | bazowy {float(sc.get('prob',0.0)):.2%}")
    ...
    lines += ["", "🚫 KURS BUKMACHERA: 0% WPŁYWU NA WYBÓR DOKŁADNEGO WYNIKU."]
...
def process_exact_watch(chat_id=None, update=False):
    """Analyze all four fixed fixtures, select the two strongest, and maintain a 30-minute watch."""
    history = load_history()
    if history.empty or len(history) < 200:
        send("❌ Brak wystarczającej historii do dokładnej analizy.", chat_id)
        return False
    ...
    all_predictions, errors = _exact_watch_predictions(history)
    if len(all_predictions) < 2:
        send("❌ Nie udało się przygotować co najmniej dwóch prognoz dokładnego wyniku.", chat_id)
        return False
    selected = all_predictions[:2]
    # Do not manufacture a double coupon when the two best fixtures are still weak.
    pair_strength = (
        float(selected[0].get("fixture_quality", 0.0)) *
        float(selected[1].get("fixture_quality", 0.0))
    )
    # Exact scores are intrinsically low-probability events. The old gate
    # multiplied two single-score probabilities and demanded 3.5%, which
    # rejected many legitimate candidates (e.g. two 15% scores = 2.25%).
    # Use a two-tier gate: strong pairs pass normally; otherwise allow a
    # soft pass only when BOTH legs have a solid internal signal. This never
    # fabricates a score and never uses bookmaker odds.
    leg_probs = [float(x.get("scores", [{}])[0].get("rank_probability", 0.0)) for x in selected]
    leg_conf = [float(x.get("score_confidence", 0.0)) for x in selected]
    leg_stability = [float(x.get("scores", [{}])[0].get("stability", 0.0)) for x in selected]
    leg_agreement = [float(x.get("base_prediction", {}).get("score_agreement", 0.0)) for x in selected]
    strong_pair = pair_strength >= EXACT_PAIR_MIN_STRENGTH
    soft_pair = (
        pair_strength >= EXACT_PAIR_SOFT_STRENGTH
        and min(leg_probs) >= 0.08
        and min(leg_conf) >= 0.60
        and min(leg_stability) >= 0.45
        and min(leg_agreement) >= 0.55
    )
    if not (strong_pair or soft_pair):
        send(
            "🛡️ EXACT DOUBLE — BRAK WYSTARCZAJĄCO MOCNEGO SYGNAŁU\n\n"
            f"Najlepsza para ma łączną siłę {pair_strength:.2%}.\n"
            f"Wyniki: {selected[0].get('best_score','?')} ({leg_probs[0]:.1%}) + "
            f"{selected[1].get('best_score','?')} ({leg_probs[1]:.1%}).\n"
            "Model nie znalazł pary spełniającej nawet miękki próg jakości.\n"
            "🚫 Nie tworzę sztucznego AKO.\n"
            "🚫 Kurs bukmachera nie wpływa na decyzję.",
            chat_id,
        )
        return False
    ...
    new_combinations = [c for c in combinations if (c.get("first"), c.get("second")) not in previous_keys]
    new_signals = _exact_watch_new_signals(previous_predictions, all_predictions, max_new=3) if update else []
    # Do not treat every unchanged ranking as a new sale-worthy signal. The first
    # update may announce at most three genuinely new/strengthened exact scores.
    send(_exact_watch_message(all_predictions, selected, combinations, update=update, errors=errors, new_combinations=new_combinations, new_signals=new_signals), chat_id)
    state.update({"enabled": True, "resolved": False, "date": str(now().date()), "last_update": now().isoformat(), "combinations": combinations, "predictions": selected, "all_predictions": all_predictions})
    _exact_watch_state_save(state)
    return True
...
def main():
    ...
    log.info(
        "Master Of AI v10 uruchomiony na porcie %s.",
        port,
    )
