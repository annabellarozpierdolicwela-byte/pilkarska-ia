...
EXACT_ENGINE_VERSION = "28.0-FIX-FIXTURES-UNDEFINED-VALENCIA-BARCELONA-EVERTON-MU"
...
def _manual_exact_watch_fixtures():
    ...
    return pd.DataFrame([
        {"home": "Valencia", "away": "Barcelona", "league": "LaLiga", "date": None},
        {"home": "Everton", "away": "Manchester United", "league": "Premier League", "date": None},
    ])
...
def _exact_watch_predictions(history):
    """Analyze the two selected upcoming fixtures and rank them by exact-score strength."""
...
def _exact_watch_message(all_predictions, selected, combinations, update=False, errors=0, new_combinations=None, new_signals=None):
    title = "🔄 AKTUALIZACJA 30-MINUTOWA" if update else "🎯 EXACT DOUBLE INTELLIGENCE — 2 WSKAZANE MECZE → 5 AKO x2"
    cal = _exact_watch_calibration_load()
    ...
    lines = [
        ...
        "",
        "🔎 RANKING 2 WSKAZANYCH MECZÓW — EXACT DOUBLE INTELLIGENCE (BEZ KURSÓW)"
    ]
...
def process_exact_watch(chat_id=None, update=False):
    """
    Analyze the two fixed fixtures and ALWAYS rank the best available exact-double pair.
...
def handle_check_command(send, chat_id):
    """Fast /check handler: checks the two active fixtures after a short delay."""
    _fast_reply(send, chat_id, "⏱️ /check — sprawdzam ostatni stan…")
    try:
        time.sleep(1.0)
        payload = {
            "engine": EXACT_ENGINE_VERSION,
            ...
            "last_check": _FAST_STATE.get("last_check_time"),
        }
