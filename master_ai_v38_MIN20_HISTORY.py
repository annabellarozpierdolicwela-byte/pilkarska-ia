...
EXACT_ENGINE_VERSION = "37.0-DEEP-HISTORY-DIAGNOSTICS-TOP5-TOP7"
...
def _api_team_history(team_name, limit=60):
    ...
    try:
        ...
        if api_budget_available():
            try:
                data = api("/fixtures", {"team": int(team_id), "last": max(100, int(limit)), "status": "FT-AET-PEN"})
        ...
        for season in seasons:
            ...
            if not api_budget_available():
                break
            try:
                data = api("/fixtures", {"team": int(team_id), "season": int(season), "last": int(limit), "status": "FT-AET-PEN"})
...
def _load_targeted_exact_history(existing):
    ...
    try:
        ...
        targets = ["Vila Nova", "Goias", "Sport Recife", "Ponte Preta"]
    ...
    # Main targeted source: Brazilian Serie B seasons.  League 4404 is the
    ...
    for a, b in (("Vila Nova", "Goias"), ("Sport Recife", "Ponte Preta")):
        if not API_KEY or not api_budget_available():
            break
...
def _manual_test_fixtures():
    """Fixed fixture pool for TEST_MODE: two user-selected Brazil matches for today."""
    today = now().date()
    rows = [
        {"home": "Vila Nova", "away": "Goias", "league": "Brazil", "status": "NS", "date": f"{today}T19:00:00+00:00", "source": "manual-test"},
        {"home": "Sport Recife", "away": "Ponte Preta", "league": "Brazil", "status": "NS", "date": f"{today}T21:00:00+00:00", "source": "manual-test"},
    ]
...
def _manual_exact_watch_fixtures():
    ...
    return pd.DataFrame([
        {"home": "Vila Nova", "away": "Goias", "league": "Brazil", "date": None},
        {"home": "Sport Recife", "away": "Ponte Preta", "league": "Brazil", "date": None},
    ])
