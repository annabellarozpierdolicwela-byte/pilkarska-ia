...
def _tsdb_league_season_history(league_id, season_label):
    """Fetch a compact season slice from TheSportsDB.
    ...
    """
    try:
        payload = tsdb_get("eventsseason.php", {"id": int(league_id), "s": season_label})
...
def _tsdb_brazil_target_history(targets, home_n=EXACT_HISTORY_VENUE_MATCHES, away_n=EXACT_HISTORY_VENUE_MATCHES, h2h_n=5):
    ...
    frames = []
    # Current season first, then previous seasons.  2019 is a safety net for
    # H2H pairs that do not reach five meetings in the last few campaigns.
    years = list(range(CURRENT_SEASON, max(2019, CURRENT_SEASON - 7) - 1, -1))
    ...
    for year in years:
        ...
        if x.empty:
            # Some TheSportsDB seasons are exposed with a split label.  Try it
            # only when the plain season label returned nothing.
            x = _tsdb_league_season_history(4404, f"{year}-{year + 1}")
        if x.empty:
            continue
    ...
    for i, pred in enumerate(selected, 1):
        ...
        try:
            ...
            if hh < EXACT_HISTORY_VENUE_MATCHES or aa < EXACT_HISTORY_VENUE_MATCHES:
                ...
                lines.append("   ✅ Pełna próbka MIN20: 20+ HOME gospodarza + 20+ AWAY gościa.")
        except Exception:
            pass
    if not selected:
        lines += [
            ...
            "Nie tworzę sztucznego AKO, ponieważ MIN20 nie został spełniony dla wymaganej historii H/A.",
