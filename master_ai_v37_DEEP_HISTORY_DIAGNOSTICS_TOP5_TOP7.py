...
def _api_team_history(team_name, limit=60):
    """Deep real team history from API-Football.

    Unlike the old fallback, this keeps fetching several accessible seasons
    until the team has enough HOME and AWAY evidence. It never invents rows.
    """
    if not API_KEY or not api_budget_available():
        return pd.DataFrame()
    try:
        ...
        rows = []
        # Current/recent seasons first. Older seasons are only a fallback.
        seasons = []
        for y in (CURRENT_SEASON, CURRENT_SEASON - 1, CURRENT_SEASON - 2, CURRENT_SEASON - 3, 2024):
            if y not in seasons:
                seasons.append(y)
        ...
        for season in seasons:
            ...
            if rows:
                ...
                h = int((tmp["home_key"] == hk).sum())
                a = int((tmp["away_key"] == hk).sum())
                if h >= 20 and a >= 20:
                    break
...
def _exact_watch_message(all_predictions, selected, combinations, update=False, errors=0, new_combinations=None, new_signals=None):
    ...
    for i, pred in enumerate(selected, 1):
        ...
        try:
            hist = pred.get("base_prediction", {}).get("history_df")
            if hist is None or not hasattr(hist, "empty") or hist.empty:
                hist = pd.DataFrame()
            if not hist.empty:
                hh = int((hist.get("home_key") == hk).sum())
                ha = int((hist.get("away_key") == hk).sum())
                ah = int((hist.get("home_key") == ak).sum())
                aa = int((hist.get("away_key") == ak).sum())
                lines.append(f"   🔬 Historia drużyn: {pred.get('home','?')} H/A {hh}/{ha} | {pred.get('away','?')} H/A {ah}/{aa}")
                if min(hh, ha, ah, aa) < 20:
                    lines.append("   ⚠️ Próbka niepełna — model NIE udaje, że ma 20/20 danych.")
                else:
                    lines.append("   ✅ Pełna próbka 20 H + 20 A dla obu drużyn.")
        except Exception:
            pass
