...
EXACT_ENGINE_VERSION = "15.0-GUSTAW-STYLE-ENSEMBLE"
...
def exact_score_intelligence_rank(pred, context=None):
    """
    v15 Exact Score engine: BASELINE -> AI RANKING -> AI LIFT, modeled on the observed score table format.
    ...
    for item in base:
        ...
        # v15 Monte-Carlo cross-check. It does not replace the analytical model.
...
def _exact_watch_message(all_predictions, selected, combinations, update=False, errors=0, new_combinations=None, new_signals=None):
    ...
    lines += [
        "",
        "🧠 V15: BASELINE → ENSEMBLE → MONTE CARLO → xG (jeśli dostępne) → AI LIFT → TOP 3 → TOP 5 AKO.",
...
def process_exact_watch(chat_id=None, update=False):
    ...
    decision = (
        ...
        f"🧮 V15: baseline vs AI + ensemble/MC/xG jest pokazywane przy każdym wyniku.\n"
