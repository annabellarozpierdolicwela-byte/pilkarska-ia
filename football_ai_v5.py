...
def label(p):
    if p >= 0.15:
        return "🔥 NAJMOCNIEJSZY SYGNAŁ"
    if p >= 0.10:
        return "🟢 MOCNY SYGNAŁ"
    if p >= 0.07:
        return "🟡 ŚREDNI SYGNAŁ"
    return "⚪ SŁABY SYGNAŁ"
...
def coupon_score(items):
    if not items:
        return 0.0
    # Avoid pretending that multiplying exact-score probabilities makes
    # a reliable "coupon probability".
    return sum(x["prob"] for x in items) / len(items)
...
def prediction_message(chosen, score):
    lines = [
        ...
        "",
        "📊 Model: forma + gole + dom/wyjazd + Elo + H2H",
        "🗂️ Dane: darmowa baza Football-Data + cache",
        "",
    ]
    ...
    for i, x in enumerate(chosen, 1):
        lines += [
            ...
            f"   📈 Prawdopodobieństwo modelu: {x['prob']:.2%}",
            f"   {x['label']}",
            f"   1X2: {x['p1x2']['home']:.0%} / "
            f"{x['p1x2']['draw']:.0%} / "
            f"{x['p1x2']['away']:.0%}",
        ...
        if x.get("top_scores"):
            top = " | ".join(
                f"{s['score']} {s['prob']:.1%}"
                for s in x["top_scores"][:3]
            )
            lines.append(f"   🔎 Top 3: {top}")
            lines.append("")

    lines += [
        f"🧠 Średnia siła typów: {score:.2%}",
        "",
...
def status_message():
    ...
    return (
        ...
        f"🎯 Maks. typów: {MAXM}\n"
        f"📊 Min. prawdopodobieństwo: {MINP:.0%}\n"
...
def handle_update(u):
    ...
    try:
        if cmd == "/start":
            ...
            if not chosen:
                send(
                    "🟡 Nie znalazłem wystarczająco mocnych typów.",
                    chat_id,
                )
                return
