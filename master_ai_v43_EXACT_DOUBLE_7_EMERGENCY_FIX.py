...
EXACT_ENGINE_VERSION = "43.0-ARG-EXACT-20H-20A-H2H-TOP5-TOP7-EMERGENCY-FIX"
...
def _load_targeted_exact_history(existing):
    """Merge a deep, real team/H2H sample for the two watched fixtures."""
    targets = []
    try:
    ...
    # The free team endpoint is useful as a freshness supplement, but it is
    # deliberately NOT counted as the source of the requested 20+20 sample.
    for team in dict.fromkeys(targets):
        x = _tsdb_team_history(team, limit=5)
        if not x.empty:
            frames.append(x)

    # Main targeted source: use the league-season collector only for the legacy
    # Brazil test. Argentina fixtures span multiple competitions, so using the
    # Brazilian Serie B league ID here would silently return zero relevant rows.
    target_keys_now = {norm_team(t) for t in targets}
    brazil_targets = {"vila nova", "goias", "sport recife", "ponte preta"}
    if target_keys_now & brazil_targets:
        targeted = _tsdb_brazil_target_history(
            targets,
            home_n=20,
            away_n=20,
            h2h_n=5,
    ...
    # API-Football is the PRIMARY deep-history source for Argentina.
    # TheSportsDB free team history is only a freshness supplement because its
    # eventslast endpoint can return just one recent match on the free key.
    if frames:
    ...
    for team in dict.fromkeys(targets):
        key = norm_team(team)
        home_count = int((tmp["home_key"] == key).sum()) if not tmp.empty else 0
        away_count = int((tmp["away_key"] == key).sum()) if not tmp.empty else 0
        # Always ask API-Football for a deep sample when MIN20 is not satisfied.
        # This is essential for leagues not covered by Football-Data.co.uk.
        if home_count < 20 or away_count < 20:
            x = _api_team_history(team, limit=100)
            if not x.empty:
                frames.append(x)
                tmp = pd.concat([tmp, x], ignore_index=True) if not tmp.empty else x.copy()
                tmp["home_key"] = tmp["home"].map(norm_team)
                tmp["away_key"] = tmp["away"].map(norm_team)

    # Final diagnostic: exactly what MIN20 sees for every watched team.
    if not tmp.empty:
        for _team in dict.fromkeys(targets):
            _k = norm_team(_team)
            _hh = int((tmp["home_key"] == _k).sum())
            _aa = int((tmp["away_key"] == _k).sum())
            log.warning("MIN20 PREFLIGHT %s => HOME=%d AWAY=%d", _team, _hh, _aa)

    # Direct H2H endpoint: fetch the actual two watched pairs, regardless of
    # competition. This is important for the Argentina test where the teams
    # are not all in the same league.
    pairs = []
    for i, a in enumerate(targets):
        for b in targets[i + 1:]:
            pairs.append((str(a), str(b)))
    watched_pairs = [(targets[0], targets[1]), (targets[2], targets[3])] if len(targets) >= 4 else pairs[:2]
    for a, b in watched_pairs:
        if not API_KEY or not api_budget_available():
            break
        h2h = _api_h2h_history(a, b, last=10)
        if not h2h.empty:
            frames.append(h2h)
...
def _exact_watch_predictions(history):
    ...
    for _, r in fixtures.iterrows():
        try:
            ...
            # HARD MIN20 QUALITY GATE. The history loader should already have
            # rescued the four watched teams from API-Football. If a deployment
            # still has a short sample, keep the diagnostic visible instead of
            # silently returning zero predictions.
            ...
            # USER-INTENT MIN20:
            # - 20 recent HOME matches for the home team
            # - 20 recent AWAY matches for the away team
            # - H2H is an additional signal when available; it is NOT a reason
            #   to kill a fixture when the teams genuinely have fewer than 5 meetings.
            #
            # The previous version incorrectly demanded 20 H + 20 A for BOTH
            # teams (80 venue rows total). That was unnecessarily strict and was
            # the direct reason valid Argentina fixtures disappeared.
            missing = []
            ...
            pred["min20_blocked"] = bool(missing)
            pred["min20_missing"] = missing
            pred["min20_counts"] = required_counts
            if missing:
                log.warning(
                    "MIN20 BLOCK %s-%s: %s", r["home"], r["away"], ", ".join(missing)
                )
                # Do not publish this fixture, but return a structured blocked
                # record so /typy can explain exactly why no pair was produced.
                result.append({
                    "home": r["home"], "away": r["away"], "league": r["league"],
                    "date": str(dt), "scores": [], "internal_scores": [],
                    "best_score": "", "score_confidence": 0.0,
                    "baseline_probability": 0.0, "ai_lift": 1.0,
                    "base_prediction": pred, "context": r.get("context") or {},
                    "history_mode": history_mode, "history_rows": history_rows,
                    "fixture_quality": 0.0, "min20_blocked": True,
                    "min20_counts": required_counts, "min20_missing": missing,
                })
                continue
...
def process_exact_watch(chat_id=None, update=False):
    ...
    if len(all_predictions) < 2:
        ...
        send(
            ...
            + ("\n".join(diag_lines) + "\n" if diag_lines else "") +
            "🛡️ MIN20: wymagane jest min. 20 meczów HOME gospodarza + 20 meczów AWAY gościa. H2H jest dodatkiem, gdy istnieje.\n"
    ...
    ranked = sorted([p for p in all_predictions if not p.get("min20_blocked") and p.get("scores")], key=fixture_rank, reverse=True)
    ...
    for p in all_predictions:
        if p.get("min20_blocked") or p.get("scores"):
            continue
