    ...
    try:
        if cmd == "/start":
            send(
                ...
                "/typy — dokładne wyniki dla 2 wskazanych meczów\n"
            ...
            send(
                "📌 KOMENDY\n\n"
                "/typy — analizuje 2 wskazane mecze i buduje 5 wariantów AKO ×2\n"
...
def worker():
    """Start only the exact-score watcher for the two manually selected fixtures."""
    watcher = threading.Thread(target=exact_watch_worker, name="exact-watch", daemon=True)
    watcher.start()
    # Do not start the legacy generic AKO scheduler. It uses a separate market
    # pipeline and can emit misleading "brak historii" messages unrelated to
