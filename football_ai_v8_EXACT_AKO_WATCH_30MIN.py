...
def _manual_exact_watch_fixtures():
    """Two user-selected fixtures for the exact-score AKO watch."""
    today = now().date()
    rows = [
        {"home": "Valencia", "away": "Real Betis", "league": "LaLiga", "status": "NS", "date": f"{today}T19:00:00+00:00", "source": "manual-exact-watch"},
        {"home": "Sabah FK", "away": "Hapoel Be'er Sheva", "league": "UEFA Champions League Qualifying", "status": "NS", "date": f"{today}T16:45:00+00:00", "source": "manual-exact-watch"},
    ]
    df = pd.DataFrame(rows)
    df["date"] = pd.to_datetime(df["date"], utc=True, errors="coerce")
    df["home_key"] = df["home"].map(norm_team)
...
def _exact_watch_predictions(history):
    """Return top 3 exact scores for each of the two watched fixtures."""
    ...
    for _, r in fixtures.iterrows():
        try:
            ...
            scores = pred.get("top_scores", [])[:EXACT_WATCH_TOP_SCORES]
            result.append({
                "home": r["home"],
                "away": r["away"],
                "league": r["league"],
...
def _exact_watch_combinations(predictions):
    """Rank all 3x3 exact-score combinations by model probability."""
    if len(predictions) != 2:
        return []
    combos = []
    for a in predictions[0]["scores"]:
        for b in predictions[1]["scores"]:
            pa = float(a.get("prob", 0.0))
            pb = float(b.get("prob", 0.0))
            combos.append({
                "first": a["score"],
                "second": b["score"],
                "probability": pa * pb,
...
def _exact_watch_message(predictions, combinations, update=False, errors=0, new_combinations=None):
    ...
    for i, sc in enumerate(predictions[0]["scores"], 1):
        lines.append(f"{i}. {sc['score']} — 📊 {float(sc['prob']):.2%}")
