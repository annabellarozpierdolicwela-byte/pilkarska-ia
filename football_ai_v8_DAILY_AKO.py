...
def process_daily_combined_signal(chat_id=None):
    """Full daily scan for today's matches and automatic combined coupons."""
    history = load_history()
    if history.empty or len(history) < 200:
        send("🎫 TYPY KOMBINOWANE\n\n❌ Brak wystarczającej historii do bezpiecznej analizy.", chat_id)
        return False
...
def handle_update(u):
    ...
    try:
        if cmd == "/start":
            ...
            send(
                "⏳ Master Of AI skanuje dzisiejsze mecze i szuka najmocniejszych rynków...",
                chat_id,
            )
            process_daily_combined_signal(chat_id=chat_id)
