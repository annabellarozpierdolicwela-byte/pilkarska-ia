...
EXACT_ENGINE_VERSION = "30.0-MARSEILLE-PARIS-FC-ESPANYOL-SEVILLA"
...
def _load_targeted_exact_history(existing):
    ...
    try:
        ...
        targets = ["Marseille", "Paris FC", "Espanyol", "Sevilla"]
    ...
    for team in dict.fromkeys(targets):
        if per_team_counts.get(norm_team(team), 0) >= 5:
            continue
        x = _api_team_history(team, limit=10)
        if not x.empty:
            frames.append(x)
...
def _manual_exact_watch_fixtures():
    ...
    return pd.DataFrame([
        {"home": "Marseille", "away": "Paris FC", "league": "Ligue 1", "date": None},
        {"home": "Espanyol", "away": "Sevilla", "league": "LaLiga", "date": None},
    ])
