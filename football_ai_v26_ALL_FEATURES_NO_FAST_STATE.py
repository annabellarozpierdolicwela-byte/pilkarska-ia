...
EXACT_ENGINE_VERSION = "26.0-ALL-MODELS-NO-FAST-STATE"
...
# v13/v15/v17 model extensions: baseline -> ensemble -> MC/xG -> extra signals.
...
def _manual_exact_watch_fixtures():
    ...
    return pd.DataFrame([
        {"home": "Barcelona", "away": "Athletic Bilbao", "league": "LaLiga", "date": None},
        {"home": "Celta Vigo", "away": "Osasuna", "league": "LaLiga", "date": None},
    ])
...
def _exact_watch_message(all_predictions, selected, combinations, update=False, errors=0, new_combinations=None, new_signals=None):
    ...
    if update:
        lines += ["", "🧠 Bot ponownie przeliczył oba mecze i porównał wynik z poprzednim skanem.", "⏱️ Kolejna kontrola za około 30 minut."]
    else:
        lines += ["🛡️ To są prognozy modelu, nie gwarancje trafienia.", "⏱️ Bot będzie automatycznie kontrolował oba mecze co 30 minut."]
    return "\n".join(lines)
