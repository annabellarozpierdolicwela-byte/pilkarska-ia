...
EXACT_ENGINE_VERSION = "45.0-FLASHSCORE-BRIDGE-TOP5-TOP7"
...
API_FOOTBALL_ENABLED = os.getenv("API_FOOTBALL_ENABLED", "0").lower() in ("1", "true", "yes", "on")
...
# Flashscore bridge: use only data obtained through an authorized export/feed.
# Direct automated scraping of Flashscore is intentionally NOT implemented.
FLASHSCORE_HISTORY_FILE = Path(os.getenv("FLASHSCORE_HISTORY_FILE", str(DATA / "flashscore_history.csv")))
FLASHSCORE_SOURCE_NAME = "flashscore-authorized-feed"
...
def api(path, params):
    ...
    if not API_FOOTBALL_ENABLED:
        raise RuntimeError("API-Football wyłączone — używany jest autoryzowany feed Flashscore / pozostałe źródła.")
    if not API_KEY:
...
def _tsdb_brazil_target_history(targets, home_n=20, away_n=20, h2h_n=5):
    """Build a real targeted sample for the watched Argentine competition teams.
    ...
    league-season endpoint (league 4404 = Brazilian Serie B (legacy fallback only)) and collect
    ...
    for year in years:
        x = _tsdb_league_season_history(4404, str(year)) if target_keys.intersection({"vila nova", "goias", "sport recife", "ponte preta"}) else pd.DataFrame()
        if x.empty:
            # Some TheSportsDB seasons are exposed with a split label.  Try it
            # only when the plain season label returned nothing.
            x = _tsdb_league_season_history(4404, f"{year}-{year + 1}") if target_keys.intersection({"vila nova", "goias", "sport recife", "ponte preta"}) else pd.DataFrame()
        if x.empty:
            continue
...
def _load_flashscore_authorized_history():
    """Load an authorized Flashscore export/feed from CSV.

    Required columns: date, home, away, hg, ag.
    Optional columns: league, source.
    No website scraping is performed here.
    """
    if not FLASHSCORE_HISTORY_FILE.exists():
        return pd.DataFrame()
    try:
        x = pd.read_csv(FLASHSCORE_HISTORY_FILE)
        required = {"date", "home", "away", "hg", "ag"}
        if not required.issubset({str(c).lower() for c in x.columns}):
            log.warning("Flashscore feed: missing required columns in %s", FLASHSCORE_HISTORY_FILE)
            return pd.DataFrame()
        lower = {str(c).lower(): c for c in x.columns}
        out = pd.DataFrame({
            "date": pd.to_datetime(x[lower["date"]], utc=True, errors="coerce"),
            "home": x[lower["home"]].astype(str).str.strip(),
            "away": x[lower["away"]].astype(str).str.strip(),
            "hg": pd.to_numeric(x[lower["hg"]], errors="coerce"),
            "ag": pd.to_numeric(x[lower["ag"]], errors="coerce"),
        })
        out["league"] = x[lower["league"]].astype(str).str.strip() if "league" in lower else ""
        out["source"] = FLASHSCORE_SOURCE_NAME
        out = out.dropna(subset=["date", "hg", "ag"])
        out["home_key"] = out["home"].map(norm_team)
        out["away_key"] = out["away"].map(norm_team)
        out = out.drop_duplicates(subset=["date", "home_key", "away_key"]).sort_values("date")
        log.info("Flashscore authorized feed: %d completed rows loaded", len(out))
        return out.reset_index(drop=True)
    except Exception as exc:
        log.warning("Flashscore feed load failed: %s", exc)
        return pd.DataFrame()


def _flashscore_min20_diagnostics(df, fixtures):
    """Return exact venue/H2H counts for the two watched fixtures."""
    rows = []
    if df is None or df.empty:
        return rows
    x = df.copy()
    x["home_key"] = x["home"].map(norm_team)
    x["away_key"] = x["away"].map(norm_team)
    for _, r in fixtures.iterrows():
        hk, ak = norm_team(r["home"]), norm_team(r["away"])
        hh = int((x["home_key"] == hk).sum())
        aa = int((x["away_key"] == ak).sum())
        h2h = int(((x["home_key"] == hk) & (x["away_key"] == ak) | (x["home_key"] == ak) & (x["away_key"] == hk)).sum())
        rows.append({"home": r["home"], "away": r["away"], "home_home": hh, "away_away": aa, "h2h": h2h})
    return rows
...
def _load_targeted_exact_history(existing):
    ...
    # First-class history bridge: authorized Flashscore export/feed.
    flash = _load_flashscore_authorized_history()
    if not flash.empty:
        flash_keys = set(flash["home_key"]).union(set(flash["away_key"]))
        if flash_keys & target_keys:
            frames.append(flash)
            diag = _flashscore_min20_diagnostics(flash, _manual_exact_watch_fixtures())
            for d in diag:
                log.info(
                    "FLASHSCORE MIN20 %s-%s => HOME=%d AWAY=%d H2H=%d",
                    d["home"], d["away"], d["home_home"], d["away_away"], d["h2h"]
                )

    # The free team endpoint is only a fallback when the authorized Flashscore
    # feed has not supplied any rows for that team.
    flash_keys_loaded = set()
    if not flash.empty:
        flash_keys_loaded = set(flash["home_key"]).union(set(flash["away_key"]))
    for team in dict.fromkeys(targets):
        if norm_team(team) in flash_keys_loaded:
            continue
        x = _tsdb_team_history(team, limit=5)
        if not x.empty:
            frames.append(x)

    # Main targeted source: Argentine competition seasons.  League 4404 is the
    # competition containing all four current test teams in 2026.
    ...
    # API-Football is now a real deep-history supplement, not a tiny fallback.
    # It is called only when the targeted source is short for a team.
    if frames:
    ...
    for team in dict.fromkeys(targets):
        ...
        if home_count >= 20 and away_count >= 20:
            continue
        x = _api_team_history(team, limit=60)
        if not x.empty:
            frames.append(x)
    ...
    # Direct H2H endpoint: the previous version reported H2H=0 even when
    # real meetings existed in API-Football. Fetch the two watched pairs.
    for a, b in (("Boca Juniors", "Central Cordoba"), ("Gimnasia Y Tiro de Salta", "Tristan Suarez")):
        if not API_KEY or not api_budget_available():
            break
