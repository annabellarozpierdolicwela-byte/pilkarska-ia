...
@app.get("/")
def health():
    return jsonify({"status":"ok","service":"Pilkarska AI v4"})
...
def worker():
    print("PIŁKARSKA AI v4 — skaner uruchomiony", flush=True)
    while True:
        try:
            scan()
        except Exception as e:
            print("BŁĄD:", repr(e), flush=True)
        time.sleep(max(1, SCAN) * 60)

def main():
    port = int(os.getenv("PORT", "10000"))
    threading.Thread(target=worker, daemon=True).start()
    print(f"Serwer HTTP nasłuchuje na 0.0.0.0:{port}", flush=True)
