...
EXACT_ENGINE_VERSION = "39.4-ARG-MIN20-HARD-GATE-HISTORY-RESERVE-DIAGNOSTICS-TOP5-TOP7"
...
def _exact_watch_predictions(history):
    ...
    for _, r in fixtures.iterrows():
        try:
            ...
            missing = [f"{k}={v}" for k, v in required_counts.items() if v < 20]
...
def _exact_watch_message(all_predictions, selected, combinations, update=False, errors=0, new_combinations=None, new_signals=None):
    ...
    for i, pred in enumerate(selected, 1):
        ...
        try:
            ...
            if min(hh, ha, ah, aa) < 20:
                lines.append("   ⚠️ Źródła nie dostarczyły jeszcze minimum 20 H + 20 A — brakujące mecze NIE są sztucznie tworzone.")
            else:
                lines.append("   ✅ Pełna próbka: minimum 20 H + 20 A dla obu drużyn.")
        except Exception:
            pass
    if not selected:
        lines += [
    ...
    lines += ["", "🎫 7 NAJMOCNIEJSZYCH PODWÓJNYCH AKO — DOKŁADNE WYNIKI"]
...
def process_exact_watch(chat_id=None, update=False):
    ...
    if len(all_predictions) < 2:
        ...
        send(
            ...
            + ("\n".join(diag_lines) + "\n" if diag_lines else "") +
            "🛡️ MIN20 jest twardym warunkiem: brak 20 H + 20 A = brak typu.\n"
