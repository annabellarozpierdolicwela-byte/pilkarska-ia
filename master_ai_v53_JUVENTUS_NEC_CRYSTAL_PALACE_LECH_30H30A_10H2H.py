...
EXACT_ENGINE_VERSION = "53.0-JUVENTUS-NEC-CRYSTAL-PALACE-LECH-30H30A-10H2H-TOP5-TOP7"
EXACT_TARGET_HOME_MIN = int(os.getenv("EXACT_TARGET_HOME_MIN", "30"))
EXACT_TARGET_AWAY_MIN = int(os.getenv("EXACT_TARGET_AWAY_MIN", "30"))
EXACT_TARGET_H2H_MIN = int(os.getenv("EXACT_TARGET_H2H_MIN", "10"))
...
# ============================================================
# MASTER OF AI — STATIC VERIFIED TEAM DATABASE v53
# ============================================================
# The watched European fixtures are first-time meetings according to current
# public H2H sources. We therefore keep the static H2H layer empty and never
# fabricate a historical meeting just to reach the requested 10-match target.
STATIC_TEAM_META = {}
STATIC_TEAM_MATCHES = []
STATIC_H2H_VERIFIED = []


# Dynamic European league history from public football datasets. This is used to build the
# real venue sample across multiple seasons rather than inventing missing matches.
def _load_openfootball_history():
    """Load real multi-season venue history for the relevant European leagues
    from public openfootball/football.json data.
    ...
    """
    targets = ["Juventus", "NEC Nijmegen", "Crystal Palace", "Lech Poznan"]
...
def _tsdb_brazil_target_history(targets, home_n=30, away_n=30, h2h_n=10):
    ...
    Guarantees, when the source contains enough historical fixtures:
      ...
      * last 10 H2H meetings for every watched pair
...
def _apify_fetch_target_history(force=False):
    """Fetch and cache FlashScore historical snapshots for today's European target games."""
    if not APIFY_TOKEN or not APIFY_HISTORY_ENABLED:
        return pd.DataFrame()
    try:
        ...
        target_keys = {norm_team(x) for x in [
            "Juventus", "NEC Nijmegen", "Crystal Palace", "Lech Poznan"
        ]}
...
def _load_targeted_exact_history(existing):
    """Merge static H2H + multi-season European history + optional FlashScore/Apify."""
    targets = ["Juventus", "NEC Nijmegen", "Crystal Palace", "Lech Poznan"]
    target_keys = {norm_team(t) for t in targets}
    frames = []
    ...
    for a_name, b_name in (
        ...
        log.warning("TARGET H2H %s-%s => %d/%d", a_name, b_name, h2h_n, EXACT_TARGET_H2H_MIN)
...
def _manual_exact_watch_fixtures():
    ...
    return pd.DataFrame([
        {"home": "Juventus", "away": "NEC Nijmegen",
         "league": "UEFA Europa League", "date": "2026-09-17T19:00:00+00:00"},
...
def team_strength(df, team, before=None):
    ...
    """
    x = recent_team_matches(df, team, n=30, before=before)
...
def predict(df, home, away, league=None, match_date=None):
    ...
    home_emp, home_n = _empirical_goal_distribution(
        df, home, "home", before=match_date, n=30
    )
    away_emp, away_n = _empirical_goal_distribution(
        df, away, "away", before=match_date, n=30
    )
...
def _exact_watch_predictions(history):
    ...
    for _, r in fixtures.iterrows():
        try:
            ...
            # MIN30 is a DATA TARGET, not a hard eligibility gate.
            # We prefer 30 HOME for the home team + 30 AWAY for the away team,
            ...
            missing = []
            if required_counts["home_h"] < EXACT_TARGET_HOME_MIN:
                missing.append(f"home_team_home={required_counts['home_h']}/{EXACT_TARGET_HOME_MIN}")
            if required_counts["away_a"] < EXACT_TARGET_AWAY_MIN:
                missing.append(f"away_team_away={required_counts['away_a']}/{EXACT_TARGET_AWAY_MIN}")
            pred["min30_blocked"] = False
            pred["min30_missing"] = missing
            pred["min30_counts"] = required_counts
            if missing:
                log.warning(
                    "MIN30 DATA SHORT %s-%s: %s — analysis continues",
...
def _exact_watch_message(all_predictions, selected, combinations, update=False, errors=0, new_combinations=None, new_signals=None):
    ...
    for i, pred in enumerate(selected, 1):
        ...
        try:
            ...
            if hh < EXACT_TARGET_HOME_MIN or aa < EXACT_TARGET_AWAY_MIN:
                lines.append("   ⚠️ Brak pełnego MIN30: wymagane 30 HOME gospodarza + 30 AWAY gościa. Brakujące mecze NIE są sztucznie tworzone.")
            else:
                lines.append("   ✅ Pełna próbka MIN30: 30+ HOME gospodarza + 30+ AWAY gościa.")
        except Exception:
            pass
    if not selected:
        lines += [
            ...
            "Nie tworzę sztucznego AKO, ponieważ MIN30 nie został spełniony dla wymaganej historii H/A.",
...
def process_exact_watch(chat_id=None, update=False):
    ...
    MIN30 is the target venue sample: 30 HOME matches for the home team + 30 AWAY matches for the away team. Exact-score candidates are then ranked relatively. Bookmaker odds are never used.
    """
    history = load_history()
    ...
    if len(all_predictions) < 2:
        ...
        for p in all_predictions:
            if p.get("min30_blocked"):
                c = p.get("min30_counts", {})
                diag_lines.append(
        ...
        send(
            ...
            + ("\n".join(diag_lines) + "\n" if diag_lines else "") +
            "📚 MIN30: cel to 30 HOME gospodarza + 30 AWAY gościa.\n"
            f"🧩 H2H: cel {EXACT_TARGET_H2H_MIN} spotkań, ale bot NIE tworzy brakujących meczów.\n"
    ...
    if not combinations:
        debug = []
        for p in selected:
            debug.append(f"{p.get('home','?')}–{p.get('away','?')}: SCORE={len(p.get('scores') or [])}, INTERNAL={len(p.get('internal_scores') or [])}, MIN30={p.get('min30_blocked', False)}")
        send(
