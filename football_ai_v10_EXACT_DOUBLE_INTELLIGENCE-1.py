...
EXACT_PAIR_MIN_STRENGTH = float(os.getenv("EXACT_PAIR_MIN_STRENGTH", "0.035"))
...
def process_exact_watch(chat_id=None, update=False):
    ...
    if pair_strength < EXACT_PAIR_MIN_STRENGTH:
        send(
            "🛡️ EXACT DOUBLE — BRAK WYSTARCZAJĄCO MOCNEGO SYGNAŁU\n\n"
            "Model przeanalizował wszystkie 4 mecze, ale zgodność modeli "
            "dla dwóch najlepszych spotkań jest zbyt niska.\n"
            "🚫 Nie tworzę sztucznego AKO tylko po to, żeby coś wysłać.\n"
            "🚫 Kurs bukmachera nie wpływa na tę decyzję.",
            chat_id,
        )
        return False
