...
def _load_targeted_exact_history(existing):
    ...
    # Remote sources are explicitly opt-in so /typy cannot hang.
    if os.getenv("EXACT_REMOTE_REFRESH", "0") == "1":
        try:
            ...
            if not apify_df.empty:
                ...
                apify_df = apify_df[
                    apify_df["home_key"].isin(target_keys) |
                    apify_df["away_key"].isin(target_keys)
                ].copy()
                if not apify_df.empty:
                    frames.append(apify_df)
        except Exception:
            log.exception("EXACT_REMOTE_REFRESH Apify failed")
        ...
        for team in targets:
            try:
                ...
                log.exception("EXACT_REMOTE_REFRESH team history failed: %s", team)
