...
EXACT_ENGINE_VERSION = "55.0-KOSOWO-IRLANDIA-AUSTRIA-IZRAEL-100H100A-20H2H-TOP5-TOP7"
...
def norm_team(name):
    ...
    aliases = {
        ...
        "ponte preta ec": "ponte preta",
        "independiente rivadavia m": "independiente rivadavia",
        "independiente rivadavia (m)": "independiente rivadavia",
        ...
        "atletico tucumán": "atletico tucuman",
        ...
        "operario ferroviário": "operario",
...
# ============================================================
# MASTER OF AI — INTERNATIONAL NATIONAL-TEAM HISTORY v55
# ============================================================
# Current exact-score test uses two UEFA Nations League fixtures:
#   Kosovo — Republic of Ireland
#   Austria — Israel
#
# History strategy:
#   1) Mart Jürisoo / martj42 international_results CC0 dataset through 2024
#   2) OpenFootball Internationals CC0 data for 2025-2026 updates
#   3) optional Apify/Flashscore history remains a supplementary source
#
# We do NOT invent missing rows. The loader selects the latest completed
# 100 HOME + 100 AWAY matches for each watched team, where available, and
# then explicitly adds at least 20 most recent H2H meetings for each pair,
# where the source contains at least that many.
#
# Bookmaker odds are never used by the prediction engine.

INTERNATIONAL_HOME_TARGET = 100
INTERNATIONAL_AWAY_TARGET = 100
INTERNATIONAL_H2H_TARGET = 20
...
# Bulk historical source: 49k+ men's full internationals through 2024.
# The repository documents this as CC0/public data and gives the columns used
# below (date, home_team, away_team, home_score, away_score, tournament, ...).
INTERNATIONAL_RESULTS_URL = (
    "https://raw.githubusercontent.com/martj42/international_results/master/results.csv"
)
INTERNATIONAL_RESULTS_CACHE = DATA / "international_results_1872_2024.csv"

# OpenFootball recent supplements. The repository is a CC0 mirror of 40k+
# men's full internationals and is updated through 2026.
OPENFOOTBALL_INT_BASE = "https://raw.githubusercontent.com/openfootball/internationals/master/"
OPENFOOTBALL_INT_FILES = [
    ("friendly/2025_friendly.txt", "Friendly", 2025),
    ("friendly/2026_friendly.txt", "Friendly", 2026),
    ("uefa_nations_league/2025_uefa_nations_league.txt", "UEFA Nations League", 2025),
    ("fifa_world_cup_qualification/2025_fifa_world_cup_qualification.txt", "FIFA World Cup qualification 2026", 2025),
    ("fifa_world_cup_qualification/2026_fifa_world_cup_qualification.txt", "FIFA World Cup qualification 2026", 2026),
    ("fifa_world_cup/2026_fifa_world_cup.txt", "FIFA World Cup 2026", 2026),
]
OPENFOOTBALL_INT_CACHE = DATA / "openfootball_recent_internationals.csv"
...
def _international_row(home, away, hg, ag, date, tournament, source):
    return {
        ...
        "hg": int(hg),
        "ag": int(ag),
        "league": str(tournament or "International"),
        "source": str(source),
    }


def _load_international_bulk_history():
    """Load the large 1872-2024 CC0 international-results CSV with caching."""
    try:
        if INTERNATIONAL_RESULTS_CACHE.exists():
            age = time.time() - INTERNATIONAL_RESULTS_CACHE.stat().st_mtime
            # The source ends in 2024, so a long cache is fine. Refresh weekly.
            if age < 7 * 24 * 3600:
                raw = pd.read_csv(INTERNATIONAL_RESULTS_CACHE)
            else:
                raw = None
        else:
            raw = None
        ...
        if raw is None:
            rr = requests.get(INTERNATIONAL_RESULTS_URL, timeout=45)
            rr.raise_for_status()
            INTERNATIONAL_RESULTS_CACHE.write_bytes(rr.content)
            raw = pd.read_csv(INTERNATIONAL_RESULTS_CACHE)

        need = {"date", "home_team", "away_team", "home_score", "away_score", "tournament"}
        if not need.issubset(raw.columns):
            log.warning("INTERNATIONAL bulk: missing columns %s", sorted(need - set(raw.columns)))
            return pd.DataFrame()

        out = pd.DataFrame({
            "date": pd.to_datetime(raw["date"], utc=True, errors="coerce"),
            "home": raw["home_team"].astype(str),
            "away": raw["away_team"].astype(str),
            "hg": pd.to_numeric(raw["home_score"], errors="coerce"),
            "ag": pd.to_numeric(raw["away_score"], errors="coerce"),
            "league": raw["tournament"].astype(str),
        ...
        log.warning("INTERNATIONAL bulk load failed: %s", exc)
        return pd.DataFrame()


def _parse_openfootball_international_text(text, tournament, default_year):
    """Parse the simple Football.TXT match lines used by OpenFootball."""
    if not text:
        return pd.DataFrame()
    ...
    date_re = re.compile(
        r"^(?:Mon|Tue|Wed|Thu|Fri|Sat|Sun)\s+([A-Z][a-z]{2})\s+(\d{1,2})$"
    )
    match_re = re.compile(
        r"^(.+?)\s+(\d+)\s*-\s*(\d+)\s+(.+?)(?:\s+@.*)?$"
    )
    ...
    for raw_line in text.splitlines():
        ...
        if dm:
            ...
            try:
                current_date = pd.Timestamp(year=int(default_year), month=month_map[mon], day=day, tz="UTC")
            except Exception:
                current_date = None
            continue
        ...
        home, hg, ag, away = mm.groups()
        home = home.strip()
        away = away.strip()
        # Ignore obvious non-team header/group lines.
        if not home or not away or len(home) > 60 or len(away) > 60:
            continue
        if home.startswith("Group ") or away.startswith("Group "):
            continue
        ...
        rows.append({
            ...
            "ag": int(ag),
            "league": tournament,
            "source": "openfootball-internationals-CC0",
        })

    return pd.DataFrame(rows)


def _load_openfootball_recent_international_history():
    frames = []
    for rel_path, tournament, year in OPENFOOTBALL_INT_FILES:
        cache_name = re.sub(r"[^A-Za-z0-9._-]+", "_", rel_path)
        cache_path = DATA / f"{cache_name}.cache"
        try:
            text = None
            ...
            if text is None:
                rr = requests.get(OPENFOOTBALL_INT_BASE + rel_path, timeout=30)
                if rr.status_code != 200:
                    log.warning("OPENFOOTBALL INT %s -> HTTP %s", rel_path, rr.status_code)
                    continue
                text = rr.text
                cache_path.write_text(text, encoding="utf-8")
            parsed = _parse_openfootball_international_text(text, tournament, year)
            if not parsed.empty:
                frames.append(parsed)
        except Exception as exc:
            log.warning("OPENFOOTBALL INT %s failed: %s", rel_path, exc)
...
def _normalise_international_history(df):
    if df is None or df.empty:
        return pd.DataFrame(columns=["date", "home", "away", "hg", "ag", "league", "source", "home_key", "away_key"])
    out = df.copy()
    out["date"] = pd.to_datetime(out["date"], utc=True, errors="coerce")
    out["hg"] = pd.to_numeric(out["hg"], errors="coerce")
    out["ag"] = pd.to_numeric(out["ag"], errors="coerce")
    out = out.dropna(subset=["date", "hg", "ag", "home", "away"])
    out["hg"] = out["hg"].astype(int)
    out["ag"] = out["ag"].astype(int)
    out["home_key"] = out["home"].map(norm_team)
    out["away_key"] = out["away"].map(norm_team)
    out = out[out["home_key"].ne("") & out["away_key"].ne("")]
    out = out.drop_duplicates(subset=["date", "home_key", "away_key", "hg", "ag"], keep="last")
    return out.sort_values("date").reset_index(drop=True)


def _static_team_database_df():
    if not STATIC_TEAM_MATCHES:
        return pd.DataFrame()
    return _normalise_international_history(pd.DataFrame(STATIC_TEAM_MATCHES))
...
def _load_openfootball_history():
    """Compatibility wrapper used by the rest of the v52 engine."""
    bulk = _load_international_bulk_history()
    recent = _load_openfootball_recent_international_history()
    frames = [x for x in (bulk, recent) if x is not None and not x.empty]
    if not frames:
        return pd.DataFrame()
    combined = _normalise_international_history(pd.concat(frames, ignore_index=True, sort=False))
    target_keys = {norm_team(t) for t in INTERNATIONAL_TARGETS}
    relevant = combined[
        combined["home_key"].isin(target_keys) | combined["away_key"].isin(target_keys)
    ].copy()
    relevant = relevant.sort_values("date").reset_index(drop=True)

    # We only need completed history before the current test fixtures.
    cutoff = pd.Timestamp("2026-09-24T18:45:00Z")
    relevant = relevant[relevant["date"] < cutoff].copy()

    for team in INTERNATIONAL_TARGETS:
        k = norm_team(team)
        h = int((relevant["home_key"] == k).sum())
        a = int((relevant["away_key"] == k).sum())
        log.warning(
            "INTERNATIONAL SOURCE %s => HOME=%d/%d AWAY=%d/%d",
            team, h, INTERNATIONAL_HOME_TARGET, a, INTERNATIONAL_AWAY_TARGET
        )
    return relevant
...
def _tsdb_brazil_target_history(targets, home_n=20, away_n=20, h2h_n=5):
    ...
    Guarantees, when the source contains enough historical fixtures:
      * last 20 HOME matches for every watched team
      * last 20 AWAY matches for every watched team
    ...
    selected = []
    # Exactly the requested recent venue samples: 20 at home + 20 away.
    for key in target_keys:
...
def _apify_fetch_target_history(force=False):
    ...
    try:
        ...
        if not items:
            return pd.DataFrame()
        target_keys = {norm_team(x) for x in [
            "Londrina", "Ponte Preta", "Nautico", "Operario"
        ]}
...
def _load_targeted_exact_history(existing):
    """Build the exact-score history for the four current national teams.

    For each watched team we keep the latest 100 HOME + latest 100 AWAY
    completed matches available in the public sources. Then we append the
    latest 20 H2H meetings for each watched pair (or all available if fewer).
    """
    target_keys = {norm_team(t) for t in INTERNATIONAL_TARGETS}
    frames = []

    static = _static_team_database_df()
    if not static.empty:
        frames.append(static)

    public_df = _load_openfootball_history()
    if not public_df.empty:
        frames.append(public_df)
        log.warning("INTERNATIONALS: loaded %d relevant rows before venue trim", len(public_df))

    # Do not call the old FlashScore/Apify bridge here. That bridge was built
    # for the previous club-team test set and can perform a long remote actor
    # run before /typy can answer. The national-team module already has its own
    # international-results source and remains fully independent of bookmaker
    # odds.

    # Existing broad history is useful only for graceful fallback; do not let
    # unrelated club matches pollute this national-team exact-score study.
    if existing is not None and not existing.empty:
        old = existing.copy()
        if "home_key" not in old.columns:
            old["home_key"] = old.get("home", pd.Series(dtype=str)).map(norm_team)
        if "away_key" not in old.columns:
            old["away_key"] = old.get("away", pd.Series(dtype=str)).map(norm_team)
        old = old[
            old["home_key"].isin(target_keys) | old["away_key"].isin(target_keys)
        ].copy()
        if not old.empty:
            frames.append(_normalise_international_history(old))
    ...
    for team in INTERNATIONAL_TARGETS:
        k = norm_team(team)
        home_rows = combined[combined["home_key"] == k].sort_values("date", ascending=False).head(INTERNATIONAL_HOME_TARGET)
        away_rows = combined[combined["away_key"] == k].sort_values("date", ascending=False).head(INTERNATIONAL_AWAY_TARGET)
        selected_parts.extend([home_rows, away_rows])

    selected = pd.concat(selected_parts, ignore_index=True, sort=False) if selected_parts else pd.DataFrame()

    # Explicit H2H reserve: at least 20 recent meetings per pair, when source
    # coverage contains that many. This may reach further back than the venue
    # windows, which is intentional because H2H is a separate evidence layer.
    for a_name, b_name in (("Kosovo", "Republic of Ireland"), ("Austria", "Israel")):
        ak, bk = norm_team(a_name), norm_team(b_name)
        h2h = combined[
            ((combined["home_key"] == ak) & (combined["away_key"] == bk)) |
            ((combined["home_key"] == bk) & (combined["away_key"] == ak))
        ].sort_values("date", ascending=False).head(INTERNATIONAL_H2H_TARGET)
        if not h2h.empty:
            selected = pd.concat([selected, h2h], ignore_index=True, sort=False)
        log.warning(
            "INTERNATIONAL H2H %s-%s => %d/%d (source available: %d)",
            a_name, b_name, len(h2h), INTERNATIONAL_H2H_TARGET, int(
                (((combined["home_key"] == ak) & (combined["away_key"] == bk)) |
                 ((combined["home_key"] == bk) & (combined["away_key"] == ak))).sum()
            )
        )

    selected = _normalise_international_history(selected)
    selected = selected.drop_duplicates(
        subset=["date", "home_key", "away_key", "hg", "ag"], keep="last"
    ).sort_values("date").reset_index(drop=True)

    for team in INTERNATIONAL_TARGETS:
        k = norm_team(team)
        h = int((selected["home_key"] == k).sum())
        a = int((selected["away_key"] == k).sum())
        log.warning(
            "TARGET HISTORY %s => HOME=%d/%d AWAY=%d/%d",
            team, h, INTERNATIONAL_HOME_TARGET, a, INTERNATIONAL_AWAY_TARGET
        )

    try:
        selected.to_csv(HISTORY_CACHE, index=False)
    except Exception:
        pass
    return selected
...
def _manual_test_fixtures():
    """Fixed fixture pool for TEST_MODE: two user-selected Argentina matches for today."""
    today = now().date()
    rows = [
        {"home": "Londrina", "away": "Ponte Preta", "league": "Brazil Série B", "status": "NS", "date": "2026-09-15T22:30:00+00:00", "source": "static-verified"},
        {"home": "Nautico", "away": "Operario", "league": "Brazil Série B", "status": "NS", "date": "2026-09-15T22:30:00+00:00", "source": "static-verified"},
    ]
...
def _manual_exact_watch_fixtures():
    """Only the two fixtures selected for the current exact-score test."""
    return pd.DataFrame([
        {"home": "Kosovo", "away": "Republic of Ireland",
         "league": "UEFA Nations League B", "date": "2026-09-24T20:45:00+02:00"},
        {"home": "Austria", "away": "Israel",
         "league": "UEFA Nations League B", "date": "2026-09-24T18:45:00+02:00"},
    ])
...
def predict(df, home, away, league=None, match_date=None):
    ...
    # Use the requested deep venue samples. The empirical scorer weights recent
    # matches more strongly, so older records contribute less without being
    # discarded from the model history.
    home_emp, home_n = _empirical_goal_distribution(
        df, home, "home", before=match_date, n=INTERNATIONAL_HOME_TARGET
    )
    away_emp, away_n = _empirical_goal_distribution(
        df, away, "away", before=match_date, n=INTERNATIONAL_AWAY_TARGET
    )
...
def _exact_watch_predictions(history):
    ...
    # The two exact-watch fixtures have fixed kickoff times in this test.
    # Do not run the generic fixture resolver here: it downloads broad league
    # schedules and was the other source of /typy delays on a fresh Render
    # instance.
    result = []
    errors = 0
    skipped_past = 0
    ...
    for _, r in fixtures.iterrows():
        try:
            ...
            # Venue history targets are DATA targets, not hard eligibility gates.
...
def _exact_watch_message(all_predictions, selected, combinations, update=False, errors=0, new_combinations=None, new_signals=None):
    ...
    for i, pred in enumerate(selected, 1):
        ...
        try:
            ...
            if hh < INTERNATIONAL_HOME_TARGET or aa < INTERNATIONAL_AWAY_TARGET:
                lines.append(
                    f"   ⚠️ Niepełna próbka celu: wymagane do {INTERNATIONAL_HOME_TARGET} HOME gospodarza + "
                    f"{INTERNATIONAL_AWAY_TARGET} AWAY gościa. Brakujące mecze NIE są sztucznie tworzone."
                )
            else:
                lines.append(
                    f"   ✅ Pełna próbka celu: {INTERNATIONAL_HOME_TARGET}+ HOME gospodarza + "
                    f"{INTERNATIONAL_AWAY_TARGET}+ AWAY gościa."
                )
        except Exception:
            pass
    if not selected:
        lines += [
            ...
            "Nie tworzę sztucznego AKO — do rankingu trafiają tylko dostępne, zweryfikowane dane.",
...
def _load_exact_watch_history_fast():
    """Fast history loader used only by /typy for the four national teams.

    The old path called load_history(), which downloaded many club-league CSVs
    before getting to the national-team data. On a fresh Render instance that
    could leave Telegram stuck after the first /typy message. This path reads the
    existing targeted cache first and otherwise loads only the international
    sources needed by the current four-team exact-score study.
    """
    target_keys = {norm_team(t) for t in INTERNATIONAL_TARGETS}

    # 1) Reuse a previously prepared targeted cache immediately.
    if HISTORY_CACHE.exists():
        try:
            cached = pd.read_csv(HISTORY_CACHE)
            cached["date"] = pd.to_datetime(cached["date"], utc=True, errors="coerce")
            ...
            cached["away_key"] = cached["away"].map(norm_team)
            cached = cached[
                cached["home_key"].isin(target_keys) | cached["away_key"].isin(target_keys)
            ].copy()
            if not cached.empty:
                return _normalise_international_history(cached)
        except Exception:
            log.exception("Nie udało się odczytać cache historii exact-watch")

    # 2) Fresh targeted build: international results only.  Do NOT call
    # load_history() and do NOT run the generic club-league download loop.
    try:
        public_df = _load_openfootball_history()
        if not public_df.empty:
            return _load_targeted_exact_history(public_df)
    except Exception as exc:
        log.warning("Szybkie ładowanie historii reprezentacji nieudane: %s", exc)

    # 3) Last local fallback, if an older cache is present but malformed.
...
def process_exact_watch(chat_id=None, update=False):
    ...
    The venue target is up to 100 HOME matches for each watched team and up to 100 AWAY matches for each watched team; it is a data target, not a hard eligibility gate. H2H target: 20+ when available. Exact-score candidates are ranked relatively. Bookmaker odds are never used.
    """
    history = _load_exact_watch_history_fast()
    ...
    if len(all_predictions) < 2:
        ...
        send(
            ...
            + ("\n".join(diag_lines) + "\n" if diag_lines else "") +
            "📚 Historia celu: ostatnie 100 HOME + 100 AWAY na drużynę; H2H: minimum 20, jeśli źródła mają taką liczbę. Brak pełnej próbki NIE blokuje analizy.\n"
