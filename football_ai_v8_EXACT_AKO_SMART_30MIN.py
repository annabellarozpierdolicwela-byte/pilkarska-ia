...
EXACT_WATCH_TOP_COMBINATIONS = int(os.getenv("EXACT_WATCH_TOP_COMBINATIONS", "3"))
...
def _manual_exact_watch_fixtures():
    """Two user-selected fixtures plus known tie context; no bookmaker odds involved."""
    today = now().date()
    rows = [
        {
            "home": "Valencia", "away": "Real Betis", "league": "LaLiga",
            "status": "NS", "date": f"{today}T19:00:00+00:00",
            "source": "manual-exact-watch",
            "context": {"type": "league_match"},
        },
        {
            "home": "Sabah FK", "away": "Hapoel Be'er Sheva",
            "league": "UEFA Champions League Qualifying", "status": "NS",
            "date": f"{today}T16:45:00+00:00",
            "source": "manual-exact-watch",
            # First leg: Hapoel Be'er Sheva won 2:1, so Sabah trail by one goal.
            "context": {"type": "two_leg_tie", "first_leg_home_goals": 1, "first_leg_away_goals": 2},
        },
    ]
    df = pd.DataFrame(rows)
    df["date"] = pd.to_datetime(df["date"], utc=True, errors="coerce")
...
def _exact_watch_predictions(history):
    """Return top 3 exact scores after intelligence re-ranking for each watched fixture."""
    ...
    for _, r in fixtures.iterrows():
        try:
            ...
            pred = predict(
                history, r["home"], r["away"],
                league=r["league"], match_date=dt
            )
...
def _exact_watch_message(predictions, combinations, update=False, errors=0, new_combinations=None):
    title = "🔄 AKTUALIZACJA 30-MINUTOWA" if update else "🎯 AKTYWNY TEST — 2 MECZE × 3 DOKŁADNE WYNIKI"
    ...
    for i, sc in enumerate(predictions[0]["scores"], 1):
        lines.append(
            f"{i}. {sc['score']} — 🧠 ranking AI: {float(sc.get('rank_probability', 0.0)):.2%} "
            f"| bazowy model: {float(sc.get('prob', 0.0)):.2%}"
        )
    lines += ["", "⚽ SABAH FK — HAPOEL BE'ER SHEVA"]
    for i, sc in enumerate(predictions[1]["scores"], 1):
        lines.append(
            f"{i}. {sc['score']} — 🧠 ranking AI: {float(sc.get('rank_probability', 0.0)):.2%} "
            f"| bazowy model: {float(sc.get('prob', 0.0)):.2%}"
        )
    lines += ["", "🎫 NAJLEPSZE ŁĄCZONE AKO x2 — DOKŁADNY WYNIK"]
    ...
    for i, c in enumerate(combinations, 1):
        ...
        lines.append(
            f"{i}. {marker}Valencia {c['first']} + Sabah {c['second']} → "
            f"modelowo {c['probability']:.4%}"
        )
    if errors:
    ...
    if update:
        lines += [
            "",
            ("🆕 Model znalazł nowe kombinacje względem poprzedniej kontroli."
             if new_combinations else
             "ℹ️ Nie znaleziono nowej kombinacji lepszej od dotychczas obserwowanych."),
            "🔎 Bot nadal śledzi oba mecze przed rozpoczęciem.",
        ]
    else:
        lines += [
            "",
            "🛡️ To są prognozy modelu, nie gwarancje trafienia.",
            "⏱️ Od teraz bot będzie automatycznie kontrolował te 2 mecze co 30 minut.",
        ]
    return "\n".join(lines)
...
def process_exact_watch(chat_id=None, update=False):
    """Analyze the two fixed fixtures and maintain the 30-minute exact-score watch."""
    history = load_history()
    if history.empty or len(history) < 200:
        send("❌ Brak wystarczającej historii do dokładnej analizy.", chat_id)
        return False
    # Resolve the previous watch after the matches are known to have finished.
    ...
    predictions, errors = _exact_watch_predictions(history)
    if len(predictions) != 2:
        send("❌ Nie udało się przygotować obu prognoz dokładnego wyniku.", chat_id)
        return False
    combinations = _exact_watch_combinations(predictions)
    state = _exact_watch_state_load()
    ...
    send(
        _exact_watch_message(
            predictions, combinations, update=update, errors=errors,
