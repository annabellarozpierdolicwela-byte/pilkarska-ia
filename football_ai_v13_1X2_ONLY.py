...
EXACT_ENGINE_VERSION = "13.0-BASELINE-AI-LIFT"
...
# v13: transparent BASELINE -> AI RANKING -> AI LIFT layer.
...
# LEGACY EXACT-SCORE MODE (not used by /typy)
...
# Legacy exact-score controller (kept for compatibility; /typy uses 1X2).
...
def exact_score_intelligence_rank(pred, context=None):
    """
    v13 Exact Score engine: BASELINE -> AI RANKING -> AI LIFT.
    ...
    for item in base:
        ...
        # Geometric ensemble: disagreement between independent views is
        # penalized instead of allowing one model to dominate.
        geo = math.exp(
            0.42 * math.log(blend) +
...
def _exact_watch_message(all_predictions, selected, combinations, update=False, errors=0, new_combinations=None, new_signals=None):
    ...
    lines += [
        "",
        "🧠 V13: BASELINE (Poisson) → AI RANKING → AI LIFT.",
...
def process_exact_watch(chat_id=None, update=False):
    ...
    decision = (
        ...
        f"🧮 V13: baseline vs AI jest pokazywane przy każdym wyniku.\n"
...
def process_1x2_predictions(chat_id=None):
    """
    User mode: analyze today's upcoming fixtures and return ONLY the predicted
    match result: home win, draw, or away win. No exact scores, markets or odds
    are shown in the Telegram message.
    """
    history = load_history()
    if history.empty or len(history) < 200:
        send(
            "❌ Brak wystarczającej historii do analizy 1X2.",
            chat_id,
        )
        return False
    ...
    if fixtures is None or fixtures.empty:
        send(
            "❌ Nie znaleziono dzisiejszych nadchodzących meczów.",
            chat_id,
        )
        return False
    ...
    for _, r in fixtures.iterrows():
        ...
        try:
            ...
            log.warning("1X2 prediction failed %s-%s: %s", home, away, exc)
            continue
    ...
    if not rows:
        send(
            "❌ Nie udało się przygotować prognoz 1X2 dla dzisiejszych meczów.",
            chat_id,
        )
        return False

    # Keep the Telegram message compact. The fixture source is already sorted
    # by kickoff time, so the first results are the nearest upcoming games.
    max_rows = int(os.getenv("RESULT_PREDICTION_LIMIT", "30"))
    rows = rows[:max(1, max_rows)]

    lines = [
        "⚽ MASTER OF AI — TYPY 1X2",
        f"📅 {now().strftime('%d.%m.%Y')}",
        "",
    ]

    for item in rows:
        kickoff = item["date"].tz_convert(TZ).strftime("%H:%M")
        lines.append(
            f"{item['home']} — {item['away']} ({kickoff})\n"
            f"➡️ TYP: {item['result']}"
        )

    lines += [
        "",
        "ℹ️ 1 = gospodarz | X = remis | 2 = goście",
        "🧠 To prognoza modelu statystycznego, nie gwarancja wyniku.",
    ]
    send("\n".join(lines), chat_id)
    return True
...
def scheduled_worker():
    """Run automatically at exactly 09:00..20:00 Europe/Warsaw and send only 1X2 results."""
    log.info("AUTO 1X2: godziny %s, strefa %s", AUTO_SEND_HOURS, TZ)
    while True:
        try:
            if not AUTO_SCAN:
                time.sleep(30)
...
def process_hourly_signal(chat_id=None, force_first=False):
    """Legacy exact-score hourly controller kept for compatibility."""
    history = load_history()
    ...
    try:
        if cmd == "/start":
            send(
                ...
                "/typy — dzisiejsze typy 1X2 (wygrana gospodarzy/remis/wygrana gości)\n"
            ...
            send(
                "📌 KOMENDY\n\n"
                "/typy — analizuje dzisiejsze mecze i podaje tylko 1X2\n"
            ...
            process_1x2_predictions(chat_id=chat_id)
...
def worker():
    # User mode: no automatic exact-score messages.
    scheduled_worker()
