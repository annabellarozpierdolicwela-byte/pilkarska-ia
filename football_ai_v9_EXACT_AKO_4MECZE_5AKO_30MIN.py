...
def exact_score_intelligence_rank(pred, context=None):
    """Re-rank score candidates using independent evidence, not bookmaker prices."""
    rows = []
    ...
    margin = float(pred.get("score_margin", 0.0))
    history_quality = min(1.0, (float(pred.get("venue_history_home", 0)) + float(pred.get("venue_history_away", 0))) / max(1.0, 2.0 * EXACT_SCORE_HISTORY_MIN))
    ...
    for item in base:
        ...
        poisson_p = float(item["poisson"])
        empirical_p = float(item["empirical"])
        rho = EXACT_WATCH_DIXON_COLES_RHO
        dc = dixon_coles_tau(score_h, score_a, float(pred["lh"]), float(pred["la"]), rho)
        dc_p = poisson_p * dc
        ctx = exact_score_context_multiplier(pred.get("home"), pred.get("away"), score_h, score_a, context)

        # Independent evidence: structural model + empirical venue distribution +
        # low-score correction + model agreement + history/pattern quality.
        structural = max(1e-9, 0.72 * dc_p + 0.28 * empirical_p)
        stability = (
            0.46 * min(structural / 0.20, 1.0)
            + 0.22 * agreement
            + 0.14 * min(max(margin, 0.0) / 0.04, 1.0)
            + 0.10 * history_quality
            + 0.08 * pattern_quality
        )
        score_fit = 0.75 * blend + 0.15 * dc_p + 0.10 * empirical_p
        intelligence = max(1e-9, score_fit * (0.76 + 0.24 * stability) * ctx)
        rows.append({
    ...
    total = sum(x["intelligence"] for x in rows) or 1.0
    for x in rows:
        x["rank_probability"] = x["intelligence"] / total
    rows.sort(key=lambda x: (x["intelligence"], x["rank_probability"], x["prob"]), reverse=True)
    return rows
...
def _exact_watch_predictions(history):
    ...
    for _, r in fixtures.iterrows():
        try:
            ...
            fixture_quality = top_prob * (0.75 + 0.15 * stability + 0.10 * min(1.0, margin / 0.10))
            result.append({
...
def _exact_watch_combinations(predictions):
    """Rank all 3x3 exact-score combinations by intelligence-adjusted probability."""
    if len(predictions) != 2:
        return []
    combos = []
    for a in predictions[0]["scores"]:
        for b in predictions[1]["scores"]:
            pa = float(a.get("rank_probability", a.get("prob", 0.0)))
            pb = float(b.get("rank_probability", b.get("prob", 0.0)))
            base_pa = float(a.get("prob", 0.0))
            base_pb = float(b.get("prob", 0.0))
            combined = pa * pb
            base_combined = base_pa * base_pb
            # Confidence bonus only rewards agreement/stability; it never uses odds.
            stability = 0.5 * (float(a.get("stability", 0.0)) + float(b.get("stability", 0.0)))
            score_quality = combined * (0.82 + 0.18 * stability)
            combos.append({
                "first": a["score"],
                "second": b["score"],
                "probability": combined,
                "base_probability": base_combined,
                "stability": stability,
                "score_quality": score_quality,
            })
    combos.sort(key=lambda x: (x["score_quality"], x["probability"], x["stability"]), reverse=True)
    return combos[:EXACT_WATCH_TOP_COMBINATIONS]


def _exact_watch_message(all_predictions, selected, combinations, update=False, errors=0, new_combinations=None):
    title = "🔄 AKTUALIZACJA 30-MINUTOWA" if update else "🎯 AKTYWNY TEST — 4 MECZE → 2 NAJLEPSZE → AKO x2"
    ...
    lines = [title, f"📅 {now().strftime('%d.%m.%Y %H:%M')} ({TZ})", cal_line, "", "🔎 RANKING 4 MECZÓW — WYBÓR TYLKO WG MODELU"]
    ...
    lines += ["", "🎫 TOP 5 ŁĄCZONYCH AKO x2 — DOKŁADNE WYNIKI"]
    ...
    lines += ["", "🚫 Kurs bukmachera NIE jest używany do wyboru dokładnych wyników."]
    if update:
        lines += ["🧠 Bot ponownie przeliczył wszystkie 4 mecze i sprawdził, czy zmienił się ranking.", "⏱️ Kolejna kontrola za około 30 minut."]
    else:
...
def process_exact_watch(chat_id=None, update=False):
    ...
    send(_exact_watch_message(all_predictions, selected, combinations, update=update, errors=errors, new_combinations=new_combinations), chat_id)
...
def status_message():
    ...
    return (
        ...
        f"🎟️ Aktywny duet: 2 mecze × 1 dokładny wynik\n"
        ...
        f"⏰ Kontrola: co 60 minut — bez wyjątków\n"
...
def main():
    ...
    log.info(
        "Master Of AI v6.5 uruchomiony na porcie %s.",
        port,
    )
